...
class RepConv_OREPA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, padding=1, dilation=1, groups=1, padding_mode='zeros', deploy=False, use_se=False, nonlinear=SiLU()):
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=SiLU(), drop=0.):
        super().__init__()
...
class SwinTransformerLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=8, shift_size=0,
                 ...
                 act_layer=SiLU(), norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class SwinV2_CSPB(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x1)
        return self.cv3(torch.cat((y1, y2), dim=1))
#原文链接：https://blog.csdn.net/qq_38668236/article/details/126735304
...
class SwinTransformerLayer_v2(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=SiLU(), norm_layer=nn.LayerNorm, pretrained_window_size=0):
        super().__init__()
        self.dim = dim
...
class CrissCross(nn.Module):
    ...
    def forward(self, x):
        ...
        concate = self.softmax(torch.cat([energy_H, energy_W], 3)).cuda()
        att_H = concate[:, :, :, 0:height].permute(0, 2, 1, 3).contiguous().view(m_batchsize * width, height, height).cuda()
        att_W = concate[:, :, :, height:height + width].contiguous().view(m_batchsize * height, width, width).cuda()
        out_H = torch.bmm(proj_value_H, att_H.permute(0, 2, 1)).view(m_batchsize, width, -1, height).permute(0, 2, 3, 1).cuda()
        out_W = torch.bmm(proj_value_W, att_W.permute(0, 2, 1)).view(m_batchsize, height, -1, width).permute(0, 2, 1, 3).cuda()
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        max = self.fc2(self.relu(self.fc1(max)))
        x = avg + max
        return self.sigmoid(x)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.sigmoid(x)
'''
class CBAMREDUCTION(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16, kernel_size=7):
        super(CBAMREDUCTION, self).__init__()
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        avg_pool = torch.mean(avg_pool, dim=-2, keepdim=True)
        max_pool, _ = torch.max(x, dim=-1, keepdim=True)
        max_pool, _ = torch.max(max_pool, dim=-2, keepdim=True)
        cam = torch.cat([avg_pool, max_pool], dim=1)
        cam = self.fc1(cam)
        cam = self.relu(cam)
        cam = self.fc2(cam)
        ...
        avg_pool = torch.mean(x, dim=1, keepdim=True)
        max_pool, _ = torch.max(x, dim=1, keepdim=True)
        sam = torch.cat([avg_pool, max_pool], dim=2)
        sam = self.conv(sam)
...
class CBAMTransformer(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=16, d_model=512, nhead=8, dim_feedforward=2048, dropout=0.1):
        super(CBAMTransformer, self).__init__()
        self.cbam = CBAM(in_channels, reduction)
        self.transformer = TransformerAttention(d_model, nhead, dim_feedforward, dropout)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_features,  n_heads=8, dropout_prob=0.1):
        super().__init__()
        ...
        self.query_projection = nn.Linear(n_features, n_features)
        self.key_projection = nn.Linear(n_features, n_features)
        self.value_projection = nn.Linear(n_features, n_features)
        ...
        self.spatial_attention = nn.Sequential(
            nn.Conv2d(in_channels=n_features, out_channels=n_features // 8, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(in_channels=n_features // 8, out_channels=1, kernel_size=1)
        )
...
#        batch_size, seq_length, n_features,  = x.size()
        batch_size, wide,high, n_features,  = x.size()
        seq_length = wide * high * 2
        ...
        attention_output = attention_output.contiguous().view(batch_size, seq_length, self.n_features)

        # Apply CBAM spatial and channel attention
        spatial_attention = self.spatial_attention(x.transpose(1, 3)).transpose(1, 3)
        channel_attention = self.channel_attention(x.mean(dim=1)).unsqueeze(2).unsqueeze(3)
        cbam_attention = torch.sigmoid(spatial_attention + channel_attention)
        cbam_output = cbam_attention * attention_output

        # Project output
        output = self.output_projection(cbam_output)
...
class TransformerAttention(nn.Module):
    def __init__(self, in_channels, out_channels, heads=8, dropout=0.1):
        ...
        self.multihead_linear = nn.Linear(out_channels, out_channels)
    ...
    def forward(self, x):
        ...
        query = self.query_conv(x).view(batch_size, self.heads, -1, height * width)
        key = self.key_conv(x).view(batch_size, self.heads, -1, height * width)
        value = self.value_conv(x).view(batch_size, self.heads, -1, height * width)

        energy = torch.matmul(query.transpose(2, 3), key)
        attention = F.softmax(energy, dim=-1)

        out = torch.matmul(self.dropout(attention), value.transpose(2, 3))
        out = out.view(batch_size, -1, height, width)
        out = self.multihead_linear(out)
        # out = self.layer_norm(out + x)
        out = self.layer_norm(out + x.unsqueeze(2))
...
class AdaCBAM(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        super(AdaCBAM, self).__init__()
        self.channel_attention = nn.Sequential(
            AdaptiveAvgPool(),
    ...
    def forward(self, x):
        x_se = self.channel_attention(x) * x
...
class AdaptiveCBAM(nn.Module):
    ...
    def forward(self, x):
        # 通道注意力计算
        channel_avg_pool = torch.mean(x, dim=3, keepdim=True)
        channel_max_pool = torch.max(x, dim=3, keepdim=True)[0]
        channel_pool = torch.cat([channel_avg_pool, channel_max_pool], dim=3)
        channel_att = self.fc2(self.relu(self.fc1(channel_pool)))
        ...
        # 空间注意力计算
        spatial_att = self.conv(x_channel)
...
class AdaChannelAttention(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        super(AdaChannelAttention, self).__init__()
        self.fc1 = nn.Conv2d(in_channels, in_channels // reduction_ratio, kernel_size=1, bias=False)
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv2d(in_channels // reduction_ratio, in_channels, kernel_size=1, bias=False)
        self.sigmoid = nn.Sigmoid()
...
class AdaSpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        super(AdaSpatialAttention, self).__init__()
        assert kernel_size in (3, 7), 'kernel size must be 3 or 7'
        padding = 3 if kernel_size == 7 else 1
        self.conv = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()
...
#     def __init__(self, in_channels, reduction_ratio=16, kernel_size=7):
#         super(AdaCBAM, self).__init__()
#         self.channel_att = AdaChannelAttention(in_channels, reduction_ratio)
#         self.spatial_att = AdaSpatialAttention(kernel_size)
#
#     def forward(self, x):
#         x_se = x * self.channel_att(x)
#         x_sa = x * self.spatial_att(x)
#         x = x_se + x_sa
#         return x